...
class ScheduledDate extends Date {
     ...
     * @param {string} region - Region to interpret the date for.
     * @param {string} format - Format to parse the date from.
     */
    constructor(date, region, format) {
        if(ScheduledDate.isValid(date)
            && ScheduledDate.isValid(format, true)
            && (typeof region == "string")
            && ScheduledDate.matchesPattern(date, format)) {
            const splitPattern = ScheduledDate.split(format);
            ...
            //TODO use tzsupport utils here instead, so we don't need to adjust the timezone

            super(year, month, day, hour, minute);

            ScheduledDate.adjustTimezone(this, region);
        }
        else {
     ...
     * @returns {undefined}
     */
    static adjustTimezone(date, region, direction = false) {
        let difference = 0;
     ...
     *
     *  @param {Date} date - Date to format, only actually does anything if it's a
     *                                Date instance.
     ...
     */
    get valid() {
        return ScheduledDate.isValid(this.rawDate) && ScheduledDate.matchesPattern(this.rawDate, this.format);
    }
}
